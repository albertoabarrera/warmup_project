...
CREATE FUNCTION assign2proj() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE 
  x int;
...
CREATE FUNCTION definedept() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN 
   INSERT INTO Dept 
...
CREATE FUNCTION insertemp() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN 
...
CREATE FUNCTION numemployees() RETURNS integer
    LANGUAGE plpgsql
    AS $$
declare
   empNum integer;
...
CREATE FUNCTION updateempcount() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN 
   UPDATE  Dept
...
CREATE FUNCTION updatesalary() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN 
   UPDATE  Emp
...
ALTER TABLE proj OWNER TO postgres;
...
ALTER TABLE projectemp OWNER TO postgres;

--
-- Data for Name: dept; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY dept (dno, dname, mgr, numemp) FROM stdin;
111	HR	Alice	2
444	Hardware	John	3
...
COPY emp (ssn, ename, dno, sal) FROM stdin;
...
COPY proj (proj_id, ptitle, startdate, enddate, numemp) FROM stdin;
...
COPY projectemp (proj_id, ssn, begindate) FROM stdin;
...
ALTER TABLE ONLY emp
    ADD CONSTRAINT emp_dno_fkey FOREIGN KEY (dno) REFERENCES dept(dno);
...
ALTER TABLE ONLY projectemp
    ADD CONSTRAINT projectemp_proj_id_fkey FOREIGN KEY (proj_id) REFERENCES proj(proj_id);
